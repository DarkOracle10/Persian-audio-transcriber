                        ...
                        if key == "library_paths" and isinstance(cuda_config[key], dict):
                            for os_key in cuda_config[key]:
                                if os_key in default_config["library_paths"]:
                                    default_config["library_paths"][os_key] = cuda_config[key][os_key]
                        else:
...
def setup_cuda_paths() -> bool:
    ...
    if platform_name == "windows":
        ...
        new_paths_str = ";".join(
            str(p) for p in paths if str(p) not in current_path
        )
        if new_paths_str:
        ...
        new_paths_str = ":".join(
            str(p) for p in paths if str(p) not in current_ld
        )
        if new_paths_str:
